...
function OnEarlyContentComplete( $buffer, $tmpUpdate = false )
{
	...
	{
		if( !headers_sent() )
			header( 'X-Seraph-Accel-Content: 2.21.10; status=' . $skipStatus . ', procTime=' . $tmProc . 's' );
...
function &GetContentProcessCtx( $serverArgs, $sett )
{
	...
	if( !$seraph_accel_g_ctxProcess )
	{
		...
		$siteWpRootSubPath = trim( substr( rtrim( Wp::GetSiteWpRootUrl( '', null, true ), '/' ), strlen( rtrim( $siteRootUrl, '/' ) ) ), '/' );
